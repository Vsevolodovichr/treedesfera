...
import { X, ImageIcon, AlertCircle, Check, ChevronRight } from 'lucide-react';
...
export default function CameraPage() {
  ...
  const { rooms, currentRoomIndex, setCurrentRoomIndex, addPhotoToRoom, updateRoom } = useStore();
  ...
  const capturePhoto = useCallback(() => {
    ...
    if (videoRef.current && canvasRef.current) {
      ...
      if (ctx) {
        ...
        let status: CapturedPhoto['status'] = 'accepted';
        const issues: string[] = [];
  ...
  }, [shotIndex, instruction, currentRoom]);
      ...
      <AnimatePresence mode="wait">
        ...
        >
          <div className="bg-[rgba(0,0,0,0.5)] backdrop-blur-lg border border-white/[0.08] rounded-[16px] px-5 py-4 text-center">
            ...
            <p className="text-[12px] text-[#888] mt-1 capitalize">{instruction.type}</p>
          </div>
        </motion.div>
      </AnimatePresence>
      ...
      <div className="absolute bottom-0 left-0 right-0 z-50 bg-gradient-to-t from-black/80 to-transparent pt-16 pb-8 px-6">
        ...
        {capturedPhotos.length >= shotInstructions.length && (
          <motion.button
            ...
            className="mt-4 w-full h-12 bg-[#d4af37] text-[#0a0a0a] font-semibold rounded-[12px] flex items-center justify-center gap-2"
          >
            {currentRoomIndex < activeRooms.length - 1 ? 'Наступна кімната' : 'Завершити зйомку'}
